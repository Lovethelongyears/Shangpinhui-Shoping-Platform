...
export default {
  ...
  computed: {
    // mapGetters里面的写法：传递的数组，因为getters计算是没有分模块【home，search】
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getData();
      // 地址栏也需要修改：进行路由的跳转
      // 删除query参数，如果路径中出现params参数不应该删除，路由跳转时应带params参数
    ...
    changeOrder(flag) {
      // flag形参：它是一个标记，代表用户点击的是综合还是价格[用户点击的时候传递进来的]
      // 最开始的状态
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起ajax请求
      this.getData();
      // 每一次请求完毕，应该把相应的1,2,3级分类清空，让它接收下一次的相应数据
